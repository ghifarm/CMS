                ...
                <!-- <div class="form-group">
                            <validation-provider rules="required|min:3" v-slot="{ dirty, valid, invalid, errors }">
                              ...
                              <div class="dropdown mb-1">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownModulCreate">
                                  <a v-for="(dev) in drop_dev" :key="dev?.uuid" class="dropdown-item" @click="chooseDev(dev)">{{
                                      dev?.name
                                  }}
                                  </a>
                                  <div class="dropdown-divider"></div>
                                  <a class="dropdown-item" @click="resetDev(user_dev)">Reset</a>
                                </div>
                                <div class="mt-1">
                                  <div>
                ...
                <div class="row">
                  <div class="form-group col">
                    <validation-provider name="Start Time" rules="required" v-slot="{ errors }">
                      ...
                      <date-picker class="col" v-model="start_timeEdit" type="time" valueType="format"
                        placeholder="HH-MM-SS"></date-picker>
                      <div class="text-danger input-group">
                        <small>{{ errors[0] }}</small>
                      </div>
                  ...
                  <div class="form-group col">
                    <validation-provider name="End Time" rules="required" v-slot="{ errors }">
                      ...
                      <date-picker class="col" v-model="end_timeEdit" type="time" valueType="format"
                        placeholder="HH-MM-SS"></date-picker>
                      <div class="text-danger input-group">
                        <small>{{ errors[0] }}</small>
                      </div>
...
export default {
  ...
  methods: {
    ...
    getAllJadwal() {
      axiosinstance.get("request/" + this.uuidUser).then((response) => {
    ...
    createForm() {
      ...
      var startEndDate = moment(currentDate).format('YYYY-MM-DD');
  ...
  computed: {
    sortedItems: function () {
      return this.dataJadwal.slice().sort((a, b) => new Date(a.start_date) - new Date(b.start_date))
    }
  },
  mounted() {
    this.getDataDev();
